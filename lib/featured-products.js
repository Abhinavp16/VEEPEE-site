...
export function normalizeFeaturedProduct(product, index = 0) {
    ...
    const images = Array.isArray(product?.images)
        ? product.images
            .map((image) => String(image?.url || image || '').trim())
            .filter(Boolean)
        : [];
    ...
    return {
        ...
        price: typeof product?.price === 'number'
            ...
            : String(product?.price || formatCatalogPrice(product?.retailPrice)).trim(),
        image: String(product?.primaryImage || product?.image || images[0] || featuredProductFallbackImage).trim(),
        images,
...
export async function getCatalogProduct(slug) {
    try {
        const response = await fetch(`${getApiBase()}/products/${encodeURIComponent(slug)}`, {
            cache: 'no-store',
        });
        if (response.ok) {
            const json = await response.json();
            if (json?.data) {
                return normalizeFeaturedProduct(json.data);
            }
        }
    } catch {
        // Fall through to curated website data when the catalogue API is unavailable.
    }

    const products = await getFeaturedProducts();
    return findFeaturedProductBySlug(products, slug) || null;
}
